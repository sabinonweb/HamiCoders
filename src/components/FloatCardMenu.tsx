...
const FloatCardMenu: React.FC<FloatCardMenuProps> = ({menuRef,isClick}) =>{
  return (
    <div className={`py-2 bg-themeBgSecondary rounded-lg shadow border-[1px] border-themeBgNotification absolute top-16  right-2 ${!isClick?'opacity-0':'opacity-100'}`} ref={menuRef}>
        {cardMenuContext.map((item)=>{
            return(<p className="flex gap-4 text-base text-themeDividerPrimary p-2 cursor-pointer hover:bg-themeBgNotification"><p className="flex-1/4">{item.icon}</p><p className="flex-3/4">{item.text}</p></p>)
        })}
    </div>
  )
}
