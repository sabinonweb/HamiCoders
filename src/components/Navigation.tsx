...
const Navigation = () => {
  ...
  useEffect(() => {
    ...
    return window.addEventListener("resize", handleMobileView);
  });
...
`}
        ...
        <motion.div
          className={`border-t border-slate-700 bottom-0 absolute w-full h-12 flex justify-between items-center`}
        >
          {mobileData.map((list) => (
            <motion.button
              className={`${
                activeButton === list.key
                  ? "bg-slate-50 h-1 mb-10 w-16 flex justify-center transition-transform duration-[9000ms]"
                  : "mb-2"
              } hover:bg-slate-500 w-28 rounded-xl`}
              onClick={() => setActiveButton(list.key)}
            >
              <FontAwesomeIcon
                key={list.key}
                icon={list.icon}
                className={`${
                  activeButton === list.key ? "text-white" : "text-slate-400"
